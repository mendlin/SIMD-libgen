...
#if (BLOCK_SIZE == 128)
#include "bitblock128.h"
#endif

/*  BLOCK_SIZE 256 for AVX */
#if (BLOCK_SIZE == 256)
#include "bitblock256.h"
#endif
...
IDISA_ALWAYS_INLINE BitBlock mask_forward_zeroes(uint32_t count) {
	if (count >= sizeof(BitBlock) * 8) return simd_constant_1(0);
		else return bitblock_sll(simd_constant_1(1), convert(count));
}
...
IDISA_ALWAYS_INLINE BitBlock mask_reverse_zeroes(uint32_t count) {
	if (count >= sizeof(BitBlock) * 8) return simd_constant_1(0);
		else return bitblock_srl(simd_constant_1(1), convert(count));
}
