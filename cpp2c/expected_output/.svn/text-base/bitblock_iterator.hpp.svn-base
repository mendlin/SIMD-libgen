...
public:
	...
	IDISA_ALWAYS_INLINE int init(const BitBlock *s) {
		remaining._bitblock = *s;
#if (FW == 8) 
                mask = hsimd_signmask_8(simd_not(simd_eq_8(simd_constant_1(0), remaining._bitblock)));
#else //FW == 32
                mask = hsimd_signmask_32(simd_not(simd_eq_32(simd_constant_1(0), remaining._bitblock)));
#endif
        }
...
/*
    ...
    (3) load the blocks;
	for (i = 0; i++; i < n) {mybitblock = ...; s.load_block(mybitblock, i);}

    (4) Iterative scan loop (generates each position exactly once.)
        while (s.has_next()) { pos = s.scan_to_next(); ...   };
*/

template <class bitblock_t, class scanblock_t, class scanfield_t, int bitblock_count>
class BitStreamScanner {
public:
        /* Make sure that the number of bits in the mask at least equals 
           the number of scanblocks. */
...
if (sizeof(scanfield_t) == 1) {
                BitBlock mask_i = simd_not(simd_eq_8(simd_constant_1(0), b));
                mask |= ((scanblock_t) hsimd_signmask_8(mask_i)) << (i * sizeof(bitblock_t)/sizeof(scanfield_t));
} else if (sizeof(scanfield_t) == 2) {
                BitBlock mask_i = simd_not(simd_eq_16(simd_constant_1(0), b));
                mask |= ((scanblock_t) hsimd_signmask_16(mask_i)) << (i * sizeof(bitblock_t)/sizeof(scanfield_t));
} else if (sizeof(scanfield_t) == 4) {
                BitBlock mask_i = simd_not(simd_eq_32(simd_constant_1(0), b));
                mask |= ((scanblock_t) hsimd_signmask_32(mask_i)) << (i * sizeof(bitblock_t)/sizeof(scanfield_t));
} else { //sizeof(scanfield_t) == 8
                BitBlock mask_i = simd_not(simd_eq_64(simd_constant_1(0), b));
                mask |= ((scanblock_t) hsimd_signmask_64(mask_i)) << (i * sizeof(bitblock_t)/sizeof(scanfield_t));
}
        }
	...
	IDISA_ALWAYS_INLINE void clear_from(int pos) {
                     ...
                     remaining._scanfield[item_pos] &= (1 << bitpos) - 1;
...
/*=============================================================================
   ...
   Deprecated: 
	...
	BitStream iterator class provide an STL Input iterator interface to scan 
	through an array of scanwords (stream) in the forward direction. 
