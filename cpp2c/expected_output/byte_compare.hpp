#ifndef BYTE_COMPARE_H
#define BYTE_COMPARE_H
...
IDISA_ALWAYS_INLINE bool compare<BitBlock>(const uint8_t * x, const uint8_t * y, const uint32_t offset) {
    BitBlock temp = simd_xor(bitblock_load_unaligned((BitBlock*) ((uint8_t *)x + offset)),
			     bitblock_load_unaligned((BitBlock*) ((uint8_t *)y + offset))); // << shift offset
    return bitblock_all(simd_not(temp));
}
