...
IDISA_ALWAYS_INLINE BitBlock interpose32_(BitBlock s, BitBlock s32) {
	return simd_or(simd_slli_32(n, s), simd_srli_32(32-n, s32));
}

template<uint32_t n>
IDISA_ALWAYS_INLINE BitBlock interpose64_(BitBlock s, BitBlock s64) {
	return simd_or(simd_slli_64(n, s), simd_srli_64(64-n, s64));
}
...
#if (BLOCK_SIZE == 128) 
	    	adc(simd_constant_128(0), simd_not(charclass), carryin, cq[carryno], marker);
#else //BLOCK_SIZE == 256
	    	adc(simd_constant_256(0), simd_not(charclass), carryin, cq[carryno], marker);
#endif
//	    	cq[carryno] = carry_flip(cq[carryno]);
	...
	{
		BitBlock markers1;
		//assert(!bitblock_any(simd_and(markers0, charclass)));
	...
	{
		pending_out = (uint32_t) mvmd_extract_32( (sizeof(BitBlock)/sizeof(pending_out))-1 , strm);
#if (BLOCK_SIZE == 128) 
		return simd_or(simd_slli_128(32, strm), mvmd_fill_128((uint64_t)pending_in));
#else //BLOCK_SIZE == 256
		return simd_or(simd_slli_256(32, strm), mvmd_fill_256((uint64_t)pending_in));
#endif
	}
	...
	template <int n> IDISA_ALWAYS_INLINE BitBlock BitBlock_advance_n_(BitBlock strm, BitBlock pending_in, uint16_t pendingno)
        {
#if (BLOCK_SIZE == 128) 
		BitBlock half_block_shifted = esimd_mergel_64(strm, pending_in);
#else //BLOCK_SIZE == 256
		BitBlock half_block_shifted = esimd_mergel_128(strm, pending_in);
#endif
		cq[CarryCount + pendingno] = bitblock_srli(BLOCK_SIZE/2, strm);
		//pending64[pendingno] = bitblock_srli(BLOCK_SIZE/2, strm);
#if (BLOCK_SIZE == 128) 
		BitBlock result = simd_or(simd_srli_64((BLOCK_SIZE/2)-n, half_block_shifted),
			       simd_slli_64(n, strm));
#else //BLOCK_SIZE == 256
		BitBlock result = simd_or(simd_srli_128((BLOCK_SIZE/2)-n, half_block_shifted),
			       simd_slli_128(n, strm));
#endif
		return result;
        }
