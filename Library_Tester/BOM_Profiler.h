...
void dump_BOM_table(BOM_Table * timer_table) {
  ...
  timestamp_t sequential_stamps[SEQ_TIMESTAMP_COUNT];
  timestamp_t timestamp_overhead;

/*  Calculate minimum overhead of timestamps.  */

  sequential_stamps[0] = read_cycle_counter();
  sequential_stamps[1] = read_cycle_counter();
  sequential_stamps[2] = read_cycle_counter();
  sequential_stamps[3] = read_cycle_counter();
  sequential_stamps[4] = read_cycle_counter();
  sequential_stamps[5] = read_cycle_counter();
  sequential_stamps[6] = read_cycle_counter();
  sequential_stamps[7] = read_cycle_counter();
  timestamp_overhead = sequential_stamps[1] - sequential_stamps[0];
  for (i = 2; i < SEQ_TIMESTAMP_COUNT; i++) {
    if (sequential_stamps[i] - sequential_stamps[i-1] < timestamp_overhead)
         timestamp_overhead = sequential_stamps[i] - sequential_stamps[i-1];
  }
//  fprintf(stderr, "timestamp_overhead = %i\n", timestamp_overhead);
  ...
  for(entry = 0; entry < total_entries; entry++){
  	accumulated_cycles = timer_table->interval_end[entry] - timer_table->interval_start[entry] - timestamp_overhead;
  	if (accumulated_cycles > 0ULL) {
// 	  cout << "accumulated_cycles =" << accumulated_cycles << "; elems = " << timer_table->interval_elems[entry] << endl;
...
#endif
#ifdef PERF_SCRIPT
    if (cum_count>total_count*0.9){
      fprintf(stderr,"%i ",cum_avg);
      break;
    }
#endif
  }           
...
void destroy_BOM_timer(BOM_Table * timer_table) {
	if(timer_table) {
		free(timer_table);
	}
